...
class Stage5FRosterReadinessTests(unittest.TestCase):
    ...
    def test_production_scope_and_required_base_roster(self) -> None:
        ...
        self.assertEqual(production["required_base_species"]["count"], 1025)
        self.assertEqual(production["required_base_species"]["ready_count"], 1025)
        ...
        self.assertEqual(production["required_base_species"]["cry_runtime_ready_count"], 1025)
        ...
        self.assertEqual(_record(self.inventory, "SPECIES_SPRIGATITO")["production"]["scope"], "IN_SCOPE")
        self.assertEqual(_record(self.inventory, "SPECIES_PECHARUNT")["national_dex_number"], 1025)
        self.assertEqual(_record(self.inventory, "SPECIES_PECHARUNT")["production"]["scope"], "IN_SCOPE")
    ...
    def test_generation_gap_table_distinguishes_authenticity(self) -> None:
        ...
        self.assertEqual(sum(rows[g]["in_scope_count"] for g in range(1, 10)), 1025)
        ...
        self.assertEqual(rows[9]["in_scope_count"], 120)
    ...
    def test_regional_and_current_scope_mega_static_contracts(self) -> None:
        ...
        self.assertEqual(production["regional_static_audit"]["in_scope_count"], 55)
        self.assertEqual(production["mega_static_audit"]["total_identity_count"], 97)
        self.assertEqual(production["mega_static_audit"]["in_scope_count"], 97)
        self.assertEqual(production["mega_static_audit"]["ready_count"], 97)
        self.assertEqual(production["mega_static_audit"]["required_gap_count"], 0)
        self.assertEqual(production["mega_static_audit"]["unique_item_trigger_count"], 92)
        self.assertEqual(production["mega_static_audit"]["bag_incremental_mega_slot_reserve"], 48)
        self.assertEqual(
            production["mega_static_audit"]["bag_capacity_classification"],
            "NONBLOCKING_FULL_ITEM_COLLECTION_DEBT",
        )
        self.assertEqual(production["mega_static_audit"]["classification_counts"], {"IN_SCOPE_READY": 97})
        ...
        self.assertEqual(_record(self.inventory, "SPECIES_MEGA_MEOWSTIC_FEMALE")["mega_runtime"]["source_form"], 1)
        self.assertEqual(_record(self.inventory, "SPECIES_MEGA_MAGEARNA_ORIGINAL")["mega_runtime"]["target_form"], 3)
        self.assertEqual(_record(self.inventory, "SPECIES_MEGA_TATSUGIRI_STRETCHY")["mega_runtime"]["target_form"], 5)
        self.assertTrue(_record(self.inventory, "SPECIES_MEGA_BAXCALIBUR")["mega_runtime"]["mega_stone_classified"])
    ...
    def test_exceptional_identities_are_explained_not_fabricated(self) -> None:
        ...
        for entry in exceptions:
            self.assertIn(entry["readiness"], {"READY", "OUT_OF_SCOPE_FOR_GAME", "RESERVED_PLACEHOLDER"})
    ...
    def test_proof_manifest_and_five_dex_scenarios_are_deterministic(self) -> None:
        ...
        self.assertEqual(manifest["scope"]["base_species_last_national_dex"], 1025)
        ...
        ] + [ROOT / "qa/scenarios/stage5fs_pecharunt_dex_boundary.json"]
        for path in paths:
