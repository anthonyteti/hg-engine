...
def resolve_stage4e_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4E through the unchanged Stage 4D world/resource graph."""
    if source.get("schema_version") != 11 or source.get("artifact_namespace") != "stage4e":
        raise RegistryError("unsupported_world_schema", "Stage 4E triangle source must use schema 11")
    ...
    resolved["schema_version"] = 11
    resolved["canonical_schema_version"] = 11
    resolved["artifact_namespace"] = "stage4e"
    return resolved
...
def resolve_stage4g_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4G through the unchanged Stage 4D world/resource graph."""
    if source.get("schema_version") != 13 or source.get("artifact_namespace") != "stage4g":
        raise RegistryError("unsupported_world_schema", "Stage 4G simplification source must use schema 13")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 13
    resolved["canonical_schema_version"] = 13
    resolved["artifact_namespace"] = "stage4g"
    return resolved


def resolve_stage4i_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4I through the unchanged Stage 4D resource graph."""
    if source.get("schema_version") != 14 or source.get("artifact_namespace") != "stage4i":
        raise RegistryError("unsupported_world_schema", "Stage 4I model-capacity source must use schema 14")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 14
    resolved["canonical_schema_version"] = 14
    resolved["artifact_namespace"] = "stage4i"
    return resolved


def resolve_stage4j_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4J through the unchanged Stage 4D resource graph."""
    if source.get("schema_version") != 15 or source.get("artifact_namespace") != "stage4j":
        raise RegistryError("unsupported_world_schema", "Stage 4J approximate-decimation source must use schema 15")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 15
    resolved["canonical_schema_version"] = 15
    resolved["artifact_namespace"] = "stage4j"
    return resolved


def resolve_stage4k_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4K through the unchanged Stage 4D resource graph."""
    if source.get("schema_version") != 16 or source.get("artifact_namespace") != "stage4k":
        raise RegistryError("unsupported_world_schema", "Stage 4K hierarchy source must use schema 16")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 16
    resolved["canonical_schema_version"] = 16
    resolved["artifact_namespace"] = "stage4k"
    return resolved


def resolve_stage4l_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4L through the unchanged Stage 4D resource graph."""
    if source.get("schema_version") != 17 or source.get("artifact_namespace") != "stage4l":
        raise RegistryError("unsupported_world_schema", "Stage 4L normal source must use schema 17")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 17
    resolved["canonical_schema_version"] = 17
    resolved["artifact_namespace"] = "stage4l"
    return resolved


def resolve_stage4m_source(
    source: dict[str, Any],
    registry_path: Path = DEFAULT_REGISTRY,
) -> dict[str, Any]:
    """Resolve Stage 4M through the unchanged Stage 4D resource graph."""
    if source.get("schema_version") != 18 or source.get("artifact_namespace") != "stage4m":
        raise RegistryError("unsupported_world_schema", "Stage 4M UV source must use schema 18")
    stage4d_view = copy.deepcopy(source)
    stage4d_view["schema_version"] = 10
    stage4d_view["artifact_namespace"] = "stage4d"
    resolved = resolve_stage4d_source(stage4d_view, registry_path)
    resolved["schema_version"] = 18
    resolved["canonical_schema_version"] = 18
    resolved["artifact_namespace"] = "stage4m"
    return resolved
